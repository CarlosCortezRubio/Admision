...
class SQLCurso 
{ 
  const AGREGAR_CURSO = "INSERT INTO Curso (idCurso, idEmpleado, idSede, idProgEstudio, idPensum, idAsignatura, idPeriodo, codCurso, nomCurso, cupMaxCurso, fecIniCurso, fecFinCurso) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ";

  const ACTUALIZAR_CURSO = "UPDATE Curso SET idCurso = ?, idEmpleado = ?, idSede = ?, idProgEstudio = ?, idPensum = ?, idAsignatura = ?, idPeriodo = ?, codCurso = ?, nomCurso = ?, cupMaxCurso = ?, fecIniCurso = ?, fecFinCurso = ? WHERE idCurso = ?";
  ...
  const CONSULTAR_CURSO_POR_IDSEDE = "SELECT * FROM Curso WHERE idSedeCurso = ?";
