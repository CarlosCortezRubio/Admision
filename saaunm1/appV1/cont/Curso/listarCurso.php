...
// private $fecFinCurso;
	function principalCurso(){
		parent::__construct ();		
		$this->campos = 'idCurso,idEmpleado,idSede,idProgEstudio,idPensum,idAsignatura,idPeriodo,codCurso,nomCurso,cupMaxCurso,fecIniCurso,fecFinCurso';
		$this->modelo = 'Curso';
		$this->modulo = 'Curso';
	...
	function guardar($request,$session){
		...
		$objCurso = new Curso ();
		$tablaResultado = $objCurso->agregarCurso($this->idCurso,$this->idEmpleado,$this->idSede,$this->idProgEstudio,$this->idPensum,$this->idAsignatura,$this->idPeriodo,$this->codCurso,$this->nomCurso,$this->cupMaxCurso,$this->fecIniCurso,$this->fecFinCurso);
	...
	function actualizar($request,$session){
		$this->inicializarAtributos($request,$this->campos);
		$objCurso = new Curso ();
		$tablaResultado = $objCurso->actualizarCurso($this->idCurso,$this->idEmpleado,$this->idSede,$this->idProgEstudio,$this->idPensum,$this->idAsignatura,$this->idPeriodo,$this->codCurso,$this->nomCurso,$this->cupMaxCurso,$this->fecIniCurso,$this->fecFinCurso,$this->idCurso);
		$objCurso->cerrarConexion();
		if ($tablaResultado > 0) {
...
$this->inicializarCombos();		$objCurso = new Curso ();
...
function inicializarCombos(){
		...
		$objSede = new Sede;
		$tablaSede = Enumerado::tabla2enumerado($objSede->consultarSede(),'1',0);
		$this->setParameter('tablaSede', $tablaSede);

		$objSede->cerrarConexion();
		$objProgEstudio = new ProgEstudio;
		$tablaProgEstudio = Enumerado::tabla2enumerado($objProgEstudio->consultarProgEstudio(),'1',0);
		$this->setParameter('tablaProgEstudio', $tablaProgEstudio);

		$objProgEstudio->cerrarConexion();
		$objPensum = new Pensum;
		$tablaPensum = Enumerado::tabla2enumerado($objPensum->consultarPensum(),'1',0);
		$this->setParameter('tablaPensum', $tablaPensum);

		$objPensum->cerrarConexion();
		$objAsignatura = new Asignatura;
		$tablaAsignatura = Enumerado::tabla2enumerado($objAsignatura->consultarAsignatura(),'1',0);
		$this->setParameter('tablaAsignatura', $tablaAsignatura);

		$objAsignatura->cerrarConexion();
		$objPeriodo = new Periodo;
		$tablaPeriodo = Enumerado::tabla2enumerado($objPeriodo->consultarPeriodo(),'1',0);
