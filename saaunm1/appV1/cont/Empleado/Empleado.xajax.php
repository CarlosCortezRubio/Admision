...
$action = new Action ();
$arrayFunciones = Array ("llenarGridEmpleado", "clickEmpleado", "vntActClave");
if (! isset ( $xajax )) {
...
function llenarGridEmpleado($campoBusqueda,$indice){
	...
	$objResponse = new xajaxResponse ();
		...
		if ($campoBusqueda != '') {
		$resultado = $objEmpleado->consultarEmpleadoX_ESPECIFICAR_METODO ( $campoBusqueda ); // EDITABLE
		} else {
		$resultado = $objEmpleado->consultarEmpleado (); // EDITABLE
		}
...
/* FIN PAGINACION */ 
		...
		if ($resultado!=null) {
			...
			$campos = array('obj_' . $chk, 'obj_' . $img3,'obj_' . $img, 'obj_' . $img1,'idEmpleado','idPersona');
