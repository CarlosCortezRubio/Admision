...
{ 
  ...
  const CONSULTAR_PERSONA_POR_DOC = "SELECT * FROM Persona INNER JOIN BANCO USING(idBanco) WHERE UPPER(docPersona) = UPPER(?)";
  
  const CONSULTAR_PERSONA_POR_NOM_Y_APE = "SELECT * FROM Persona
        WHERE   UPPER(nom1Persona) LIKE UPPER(?) OR UPPER(nom2Persona) LIKE UPPER(?) OR 
                UPPER(ape1Persona) LIKE UPPER(?) OR UPPER(ape2Persona) LIKE UPPER(?)
        ORDER BY ape1Persona";
