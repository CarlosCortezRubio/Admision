...
$idBenefactorPersona = parent::prepareParam($idBenefactorPersona, Query::DATATYPE_INTEGER);
	...
	function consultarPersonaXparam($campBusq)
	{
	    ...
	    if ($matCampBusq[1]=='') {
	        return $this->consultarPersona();
	    }
	    //         dump($matCampBusq);die();
	    if ($matCampBusq[0]==1) {
	        return $this->consultarPersonaXdocPersona($matCampBusq[1]);
	    }else{
	        $params = array(
	            '%'.$matCampBusq[1].'%',
	            '%'.$matCampBusq[1].'%',
	        ...
	        return parent::executeQuery(SQLPersona::CONSULTAR_PERSONA_POR_NOM_Y_APE, $params);
	    }
