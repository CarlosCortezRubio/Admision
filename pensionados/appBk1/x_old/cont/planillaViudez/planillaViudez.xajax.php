...
function llenarGridPlanillaPensionado($campoBusqueda, $indice)
{
    ...
    $nomPagina = 'planillaViudez'; // EDITABLE
    $resultado = null;
    $numPag = 0;
    $paginas = '';
    try {
...
//             $resultado = $objPlanillaPensionado->consultarPlanillaPensionadoX_ESPECIFICAR_METODO($campoBusqueda); // EDITABLE
            $resultado = $objPlanillaPensionado->consultarPlanillaPensionadoXparam($campoBusqueda,2); // EDITABLE
        } else {
            $resultado = $objPlanillaPensionado->consultarPlanPensConsolidadoXidTipoPlanilla(2); // EDITABLE
        }
...
//             );
            ...
            $campos = array('obj_' . $chk,'obj_' . $img3,'obj_' . $img4,'obj_' . $img1,'anioPeriodo','mesPeriodo','docPersona','nom1Persona','nomPeriodo','tipRegPersona','resPenDefPersona',
...
function clickPlanillaPensionado($id, $ind)
{
    ...
    switch ($ind) {
        case 1:
            $objResponse->script("vntConsultar('planillaViudez/consultar/$id', 'Consultar PlanillaPensionado');");
            break;
        case 2:
            ;
            break;
