...
$aux = $opt; 
		$objAula = new Aula ();
		$tabla = $objAula->consultarAulaXidAula($aux);
		$objAula->cerrarConexion();
		if ($tabla > 0) {
			$etiquetas = array('id', 'idSede', 'cod', 'nom', 'desc');
			$campos = array('idAula', 'idSede', 'codAula', 'nomAula', 'descAula');
