...
// private $ctaEmpleado;
	...
	function guardar($request,$session){
		...
		$tablaResultado = $objEmpleado->agregarEmpleado(
			$this->idEmpleado,
			$this->idPersona,
			$this->idSituacion,
			$this->idTipRegistro,
			...
			$this->idDirEncargo,
			$this->codModular,
			$this->fecIngAdmPubEmpleado,
			$this->fecIngMedEmpleado,
			$this->fecCeseEmpleado,
			$this->infCompEmpleado,
			$this->fecHabEmpleado,
			$this->fecDesEmpleado,
			$this->fecFallEmpleado,
			$this->valPorEmpleado,
			$this->fecValEmpleado,
			$this->docRefEmpleado,
			$this->numSegSaludEmpleado,
			$this->eVidaEmpleado,
			$this->CUSPPEmpleado,
			$this->fecAfiPenEmpleado,
			$this->fecDevPenEmpleado,
			$this->plazaEmpleado,
			$this->tipPlazaEmpleado,
			$this->asigInserEmpleado,
			$this->diasTrabEmpleado,
			$this->numDepEmpleado,
			$this->numDecEmpleado,
			$this->licDiasEmpleado,
			$this->tipServEmpleado,
			$this->fecIniLicEmpleado,
			$this->insasitEmpleado,
			$this->permisosEmpleado,
			$this->huelgasEmpleado,
			$this->leyMenEmpleado,
			$this->leyPerEmpleado,
			$this->cadPresupEmpleado,
			$this->ctaEmpleado,
			$this->sueldo,
			$request['preG']
		);	
	...
	function actualizar($request,$session){
		$this->inicializarAtributos($request,$this->campos);
		$objEmpleado = new Empleado ();
		$tablaResultado = $objEmpleado->actualizarEmpleado(
			$this->idEmpleado,
			$this->idPersona,
			$this->idSituacion,
			$this->idTipRegistro,
	...
	function verEditar($request,$session,$opcionMod=false){
		...
		if ($tablaResultado) {
			...
			if ($opcionMod) {
				...
				//script ( "document.getElementById('codModular').value = '$idEmpleado'.concat('$dniPersona');" );
				$this->ver('Empleado.tpl');
			}
		}else{
//		$this->visualizar('principal/error.tpl');
	...
	function inicializarCombos()
    {
        ...
        $tablaCodFiscal = Enumerado::tabla2enumerado($objCodFiscal->consultarCodigoFiscal(), '2', 0);
        ...
        $tablaCodRegional = Enumerado::tabla2enumerado($objCodRegional->consultarCodigoRegional(), '2', 0);
		...
		//NEC
		$objNEC= new NEC();
        $tablaNEC = Enumerado::tabla2enumerado($objNEC->consultarNEC(), '2', 0);
		...
		//NIVEL
		$objNivel= new Nivel();
        $tablaNivel = Enumerado::tabla2enumerado($objNivel->consultarNivel(), '2', 0);
        $this->setParameter('tablaNivel', $tablaNivel);
        $objNivel->cerrarConexion();

		//Establecimiento
		$objEstablecimiento= new Establecimiento();
        $tablaEstablecimiento = Enumerado::tabla2enumerado($objEstablecimiento->consultarEstablecimiento(), '2', 0);
        ...
        $tablaRegLaboral = Enumerado::tabla2enumerado($objRegLaboral->consultarRegLaboral(), '2', 0);
        $this->setParameter('tablaRegLaboral', $tablaRegLaboral);
        $objRegLaboral->cerrarConexion();

		// Tipo Servidor
		$objTipServidor= new TipServidor();
        $tablaTipServidor = Enumerado::tabla2enumerado($objTipServidor->consultarTipServidor(), '2', 0);
        $this->setParameter('tablaTipServidor', $tablaTipServidor);
        $objTipServidor->cerrarConexion();

		// Num Magis 
		$objNumMagist= new NumMagist();
        $tablaNumMagist = Enumerado::tabla2enumerado($objNumMagist->consultarNumMagist(), '2', 0);
        ...
        $tablaGpoRemunerativo = Enumerado::tabla2enumerado($objGpoRemunerativo->consultarGpoRemunerativo(), '2', 0);
        $this->setParameter('tablaGpoRemunerativo', $tablaGpoRemunerativo);
        $objGpoRemunerativo->cerrarConexion();

		//Hat EMb
		$objHatEmb= new HatEmb();
        $tablaHatEmb = Enumerado::tabla2enumerado($objHatEmb->consultarHatEmb(), '2', 0);
        ...
        $tablaCargoOrigen = Enumerado::tabla2enumerado($objCargoOrigen->consultarCargoOrigen(), '2', 0);
        ...
        $tablaNivelOrigen = Enumerado::tabla2enumerado($objNivelOrigen->consultarNivelOrigen(), '2', 0);
        ...
        $tablaPalMagist = Enumerado::tabla2enumerado($objPalMagist->consultarPalMagist(), '2', 0);
