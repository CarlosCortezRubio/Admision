...
// private $DESCROL;
	function principalRol(){
		parent::__construct ();		
		$this->campos = 'IDROL,Edo_idEdoRegistro,NOMROL,DESCROL';
		$this->modelo = 'Rol';
		$this->modulo = 'Rol';
	...
	function guardar($request,$session){
		...
		$objRol = new Rol ();
		$tablaResultado = $objRol->agregarRol($this->IDROL,$this->Edo_idEdoRegistro,$this->NOMROL,$this->DESCROL);
	...
	function actualizar($request,$session){
		$this->inicializarAtributos($request,$this->campos);
		$objRol = new Rol ();
		$tablaResultado = $objRol->actualizarRol($this->IDROL,$this->Edo_idEdoRegistro,$this->NOMROL,$this->DESCROL,$this->idRol);
		$objRol->cerrarConexion();
		if ($tablaResultado > 0) {
...
function ver($pagina, $formatoImpresion = false){
	...
	function ventanaConsultar($request,$session){
		...
		$this->inicializarAtributos($request,'id');
		$objRol = new Rol ();
		$tablaResultado = $objRol->consultarRolXid($this->id);
		$objRol->cerrarConexion();
		if ($tablaResultado) {
				$this->asignarParametros($request,'vr',$this->modelo,$opcion,null,$tablaResultado,$this->campos);
				$this->setParameter ( 'etiquetaModulo', 'Consultar'. ' ' .$this->modulo);
...
//		$this->visualizar('principal/error.tpl');
	...
	function datosTabla($request,$session){
        ...
        $etiquetas = array(
            ...
            "Imp",
            "Ver",
            "Editar","IDROL Rol","Edo_idEdoRegistro Rol","NOMROL Rol","DESCROL Rol"
        );
        ...
        $campos = array(
            ...
            "obj_" . $imgImp,
            "obj_" . $imgVer,
            "obj_" . $imgEdi,"IDROL","Edo_idEdoRegistro","NOMROL","DESCROL"
        );
//         echo "$encabezado"; // Permite imprimir un titulo al grid
        ...
        $objRol = new Rol();
        $tablaResultado = $objRol->consultarRol(); // EDITABLE
        $objRol->cerrarConexion();
