...
//     const CONSULTAR_PLANPENS = "SELECT * FROM PlanPensCon WHERE edoPlanPens = 1 ORDER BY idPlanPens";
    ...
    const CONSULTAR_PLANPENS_POR_ANIO_Y_MES = "SELECT @rownum:=@rownum+1 nro, t.* FROM PlanPensCon t, (SELECT @rownum:=0) r WHERE anio = ? AND mes = ? AND edoPlanPens = 1 ORDER BY idPlanPens";
    ...
    const CONSULTAR_PLANPENS_TOTALES_POR_ANIO_Y_MES = "SELECT sum(total_imponiblePlanPens) AS totales_imponiblePlanPens,sum(total_haberPlanPens) AS totales_haberPlanPens,sum(total_descuentosPlanPens) AS totales_descuentosPlanPens,sum(total_liquidoPlanPens) AS totales_liquidoPlanPens FROM PlanPensCon t, (SELECT @rownum:=0) r WHERE edoPlanPens = 1 AND anio = ? AND mes = ? ORDER BY idPlanPens";
