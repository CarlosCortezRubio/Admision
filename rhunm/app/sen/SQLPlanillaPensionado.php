...
const CONSULTAR_PLANILLAPENSIONADO_POR_IDPLANILLAPENSIONADO = "SELECT * FROM planillapensionado 
  ...
  //const CONSULTAR_LISTADO_PLANPENS_POR_ANIO_MES_Y_TIPO = "SELECT @rownum:=@rownum+1 nro FROM planillapensionado t 
  const CONSULTAR_LISTADO_PLANPENS_POR_ANIO_MES_Y_TIPO = "SELECT @rownum:=@rownum+1 nro, t.*, P.* FROM planillapensionado t   
      INNER JOIN empleado USING(idEmpleado)
      INNER JOIN persona P USING(idPersona) 
      INNER JOIN periodo USING(idPeriodo) 
      WHERE anioPeriodo = ? AND mesPeriodo = ? ORDER BY idPlanillaPensionado";
      //WHERE anioPeriodo = ? AND mesPeriodo = ? AND idTipoPlanilla = ? ORDER BY idPlanillaPensionado";
 const CONSULTAR_PLANPENS_TOTALES_POR_ANIO_Y_MES_Y_TIPO = "SELECT 
        sum(totImpplanillaPensionado) AS totales_imponiblePlanPens,sum(totHabplanillaPensionado) AS totales_haberPlanPens, 
        sum(totDescplanillaPensionado) AS totales_descuentosPlanPens,
        sum( totLiqplanillaPensionado) AS totales_liquidoPlanPens 
        FROM planillapensionado t, periodo P, (SELECT @rownum:=0) r       
        WHERE anioPeriodo = ? AND mesPeriodo = ? ORDER BY idplanillaPensionado";
        //WHERE anioPeriodo = ? AND mesPeriodo = ? AND idTipoPlanilla = ? ORDER BY idplanillaPensionado";

 const CONSULTAR_PLANPENS_CONSOLIDADO_POR_ANIO_MES_Y_TIPO = "SELECT etiConceptos, idplanillaPensionado, idConceptos, idTipoConcepto, sum(mntConcPlanilla) as sumConcepto,
        COUNT(IF(mntConcPlanilla>0,1,NULL)) AS cantConceptos
        FROM planillapensionado 
        INNER JOIN periodo USING(idPeriodo) 
        INNER JOIN concplanilla USING(idplanillaPensionado) INNER JOIN
