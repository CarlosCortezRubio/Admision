...
function llenarGridPlanPens($campoBusqueda, $indice)
{
    ...
    $nomPagina = 'PlanPensOrf'; // EDITABLE
    $resultado = null;
    $numPag = 0;
    $paginas = '';
...
//     return $objResponse;
    try {
        ...
        if ($campoBusqueda != '') {
            $resultado = $objPlanPensOrf->consultarPlanPensOrfXparam($campoBusqueda); // EDITABLE
        } else {
            $resultado = $objPlanPensOrf->consultarPlanPensOrf(); // EDITABLE
        }
        $objPlanPensOrf->cerrarConexion(); // EDITABLE
        /* INICIO PAGINACION */
        $action = new Action();
...
function llenarGridPlanPensNom($campoBusqueda, $indice)
{
    ...
    try {
        $objPlanPensOrf = new PlanPensOrf();
        if ($campoBusqueda != '') {
            $resultado = $objPlanPensOrf->consultarPlanPensOrfXapellidos_y_nombresPlanPensOrfAprox($campoBusqueda); // EDITABLE
        } else {
            $resultado = $objPlanPensOrf->consultarPlanPensOrf(); // EDITABLE
        }
        $objPlanPensOrf->cerrarConexion(); // EDITABLE
...
function buscarDocente($campoBusqueda)
{
    ...
    try {
        $objPlanPensOrf = new PlanPensOrf();
        if ($campoBusqueda != '') {
            $resultado = $objPlanPensOrf->consultarPlanPensOrfXdniPlanPensOrf($campoBusqueda); // EDITABLE
        }
        if ($resultado != null) {
...
function clickPlanPens($id, $ind)
{
    ...
    switch ($ind) {
        case 1:
            $objResponse->script("vntConsultar('PlanPensOrf/consultar/$id', 'Consultar PlanPensOrf');");
            break;
        case 2:
            ;
            break;
