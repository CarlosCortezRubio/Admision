...
FROM planpens where idPeriodo = ?";
    ...
    const ELIMINAR_PLANPENS = "DELETE FROM PlanPensConCon WHERE idPlanPens = ?";
    
    const ACTUALIZAR_EDO_PLANPENS = "UPDATE PlanPensCon SET edoPlanPens = ? WHERE idPlanPens = ?";

//     const CONSULTAR_PLANPENS = "SELECT * FROM PlanPensConCon WHERE edoPlanPens = 1 ORDER BY idPlanPens";
    const CONSULTAR_PLANPENS_DEF = "SELECT @rownum:=@rownum+1 nro, anio,mes,dniPlanPens,apellidos_y_nombresPlanPens,planillaPlanPens,tipo_de_regimenPlanPens,resolucion_de_pension_definitivaPlanPens,total_imponiblePlanPens,total_haberPlanPens,total_descuentosPlanPens,total_liquidoPlanPens FROM PlanPensConCon t, (SELECT @rownum:=0) r WHERE edoPlanPens = 1 ORDER BY idPlanPens";
    
    
    const CONSULTAR_PLANPENS = "SELECT @rownum:=@rownum+1 nro, t.* FROM PlanPensConCon t, (SELECT @rownum:=0) r WHERE edoPlanPens = 1 ORDER BY idPlanPens";
    const CONSULTAR_PLANPENS_TOTALES = "SELECT sum(total_imponiblePlanPens) AS totales_imponiblePlanPens,sum(total_haberPlanPens) AS totales_haberPlanPens,sum(total_descuentosPlanPens) AS totales_descuentosPlanPens,sum(total_liquidoPlanPens) AS totales_liquidoPlanPens FROM PlanPensConCon t, (SELECT @rownum:=0) r WHERE edoPlanPens = 1 ORDER BY idPlanPens";

    const CONSULTAR_PLANPENS_POR_IDPLANPENS = "SELECT * FROM PlanPensConCon WHERE idPlanPens = ?";
