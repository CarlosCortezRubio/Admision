...
function llenarGridPlanPens($campoBusqueda, $indice)
{
    ...
    $nomPagina = 'PlanPensCon'; // EDITABLE
    $resultado = null;
    $numPag = 0;
    $paginas = '';
...
//     return $objResponse;
    try {
        ...
        if ($campoBusqueda != '') {
            $resultado = $objPlanPensCon->consultarPlanPensConXparam($campoBusqueda); // EDITABLE
        } else {
            $resultado = $objPlanPensCon->consultarPlanPensCon(); // EDITABLE
        }
        $objPlanPensCon->cerrarConexion(); // EDITABLE
        /* INICIO PAGINACION */
        $action = new Action();
...
function llenarGridPlanPensNom($campoBusqueda, $indice)
{
    ...
    try {
        $objPlanPensCon = new PlanPensCon();
        if ($campoBusqueda != '') {
            $resultado = $objPlanPensCon->consultarPlanPensConXapellidos_y_nombresPlanPensConAprox($campoBusqueda); // EDITABLE
        } else {
            $resultado = $objPlanPensCon->consultarPlanPensCon(); // EDITABLE
        }
        $objPlanPensCon->cerrarConexion(); // EDITABLE
...
function buscarDocente($campoBusqueda)
{
    ...
    try {
        $objPlanPensCon = new PlanPensCon();
        if ($campoBusqueda != '') {
            $resultado = $objPlanPensCon->consultarPlanPensConXdniPlanPensCon($campoBusqueda); // EDITABLE
        }
        if ($resultado != null) {
...
function clickPlanPens($id, $ind)
{
    ...
    switch ($ind) {
        case 1:
            $objResponse->script("vntConsultar('PlanPensCon/consultar/$id', 'Consultar PlanPensCon');");
            break;
        case 2:
            ;
            break;
